...
-- 创建数据库
CREATE DATABASE IF NOT EXISTS community_health DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_unicode_ci;
...
-- ============================================================
-- 表 6：居民账号绑定表 (一位居民最多绑定一个登录用户)
-- 说明：用于居民自助查看本人健康档案，保持 users 与 residents 解耦，
-- 同时满足 3NF，不把 resident_id 直接塞进 users 主表。
-- ============================================================
CREATE TABLE IF NOT EXISTS `resident_accounts` (
    `id` INT AUTO_INCREMENT PRIMARY KEY COMMENT '主键',
    `user_id` INT NOT NULL UNIQUE COMMENT '系统用户ID',
    `resident_id` INT NOT NULL UNIQUE COMMENT '居民ID',
    ...
    CONSTRAINT `fk_ra_resident` FOREIGN KEY (`resident_id`) REFERENCES `residents` (`id`) ON DELETE CASCADE
) ENGINE = InnoDB COMMENT = '居民账号绑定表';
...
VALUES ('管理员', '系统管理员，拥有全部权限'),
    ('医生', '社区医生，可录入健康数据和随访'),
    ('护士', '社区护士，可录入健康测量数据'),
    ('居民', '居民本人，仅查看与本人绑定的健康档案');
