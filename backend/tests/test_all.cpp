 ...
 *   cd backend && cmake -B build && cmake --build build --target
 * community_health_tests
 ...
 *   TEST_DB_PASS  （必须显式设置，无默认值；对应 sql/permissions.sql 中
...
inline bool summary() {
  ...
  std::cout << "测试结果: " << passed << "/" << total << " 通过"
            << (failed > 0 ? "，" + std::to_string(failed) + " 失败" : "")
            << std::endl;
  std::cout << "========================================\n";
  return (failed == 0);
}
...
static std::string uniqueTestUsername(const std::string &prefix) {
  return prefix + "_" + std::to_string(std::time(nullptr)) + "_" +
...
void testResponseHelperSuccess() {
  ...
  // 1. 默认调用：code=200，msg="操作成功"，data=null
  {
    ...
    EXPECT_EQ("success() msg 字段为 '操作成功'", j["msg"].get<std::string>(),
              std::string("操作成功"));
  ...
  // 2. 带业务数据
  {
    ...
    EXPECT_EQ("success(data) data.name 字段正确",
              j["data"]["name"].get<std::string>(), std::string("张三"));
  }

  // 3. 自定义 msg
  {
    json j = ResponseHelper::success(nullptr, "登录成功");
    EXPECT_EQ("success(data, msg) msg 字段为 '登录成功'",
              j["msg"].get<std::string>(), std::string("登录成功"));
  }
...
void testResponseHelperError() {
  ...
  // 1. 400 参数错误
  {
    ...
    EXPECT_EQ("error(400) msg 正确", j["msg"].get<std::string>(),
              std::string("参数错误"));
...
void testResponseHelperSendJson() {
  ...
  // ok() → HTTP 200，body 含 code:200
  {
    ...
    EXPECT_EQ("ok() body.msg='操作成功'", body["msg"].get<std::string>(),
              std::string("操作成功"));
  ...
  // ok(res, data, msg)
  {
    ...
    json body = json::parse(res.body);
    EXPECT_EQ("ok(data,msg) body.msg='登录成功'",
              body["msg"].get<std::string>(), std::string("登录成功"));
    EXPECT_EQ("ok(data,msg) data.user_id=42",
              body["data"]["user_id"].get<int>(), 42);
  }
  ...
  // fail(400) → HTTP 400
  {
    ...
    EXPECT_EQ("fail(400) body.msg 正确", body["msg"].get<std::string>(),
              std::string("缺少必填字段"));
  }
  ...
  // fail(503) → HTTP 500（所有 >=500 的业务码映射到 HTTP 500）
  {
    ...
    EXPECT_EQ("fail(503) HTTP status=500（>=500 均映射到 500）", res.status,
              500);
    json body = json::parse(res.body);
    // 业务 code 字段保持原始 503
...
void testResponseHelperCors() {
  ...
  EXPECT_CONTAINS("CORS Allow-Methods 含 DELETE",
                  res.get_header_value("Access-Control-Allow-Methods"),
                  "DELETE");

  EXPECT_CONTAINS("CORS Allow-Headers 含 Content-Type",
                  res.get_header_value("Access-Control-Allow-Headers"),
                  "Content-Type");
...
void testDatabaseManagerIntegration() {
  ...
  EXPECT_THROW("错误凭据 init() 应抛出 runtime_error", [&]() {
    DatabaseManager::getInstance().init(host, port, user, "WRONG_PASSWORD_XYZ",
                                        db);
  });
  ...
  EXPECT_NO_THROW("getConnection() 不应抛出", []() {
    ...
    if (!c)
      throw std::runtime_error("conn 为 nullptr");
  });
  ...
  // ── 8. query() — 查询 roles 表 ──────────────────────────
  {
    ...
    try {
      MYSQL_RES *res =
          DatabaseManager::getInstance().query("SELECT COUNT(*) FROM roles");
      if (res) {
        MYSQL_ROW row = mysql_fetch_row(res);
        // init.sql 插入了 3 条角色：管理员、医生、护士
        // 用 atoi 避免 stoi 在非数字串时抛出异常
        ok = row && row[0] && std::strlen(row[0]) > 0 && std::atoi(row[0]) >= 3;
        mysql_free_result(res);
      }
    } catch (...) {
    }
  ...
  // ── 10. execute() — INSERT / DELETE ─────────────────────
  {
    ...
    try {
      ...
      std::string escape_name =
          DatabaseManager::getInstance().escape("测试社区_unittest");
      DatabaseManager::getInstance().execute(
          "INSERT INTO communities (name, address) VALUES ('" + escape_name +
          "', 'unit-test-addr')");
  ...
  // ── 11. lastInsertId() — 自增 ID 大于 0 ─────────────────
  {
    ...
    try {
      std::string name =
          DatabaseManager::getInstance().escape("自增测试社区_xyz");
      DatabaseManager::getInstance().execute(
...
void testAuthPermissionClosure() {
  const std::string host = testEnv("TEST_DB_HOST", "127.0.0.1");
  const int port = testEnvInt("TEST_DB_PORT", 3306);
  const std::string user = testEnv("TEST_DB_USER", "ch_admin");
  const std::string pass = testEnv("TEST_DB_PASS", "");
  ...
  if (pass.empty()) {
    std::cout << "  [SKIP] TEST_DB_PASS 未设置，跳过权限闭环测试。\n";
    return;
  }
  ...
  const std::string activeUsername = uniqueTestUsername("active_doctor");
  const std::string disabledUsername = uniqueTestUsername("disabled_doctor");
  const std::string adminUsername = uniqueTestUsername("admin_user");
  const std::string doctorRoleCheckUsername = uniqueTestUsername("role_doctor");
  const std::string password = "123456";

  const int activeUserId = userDao.createUser(
      activeUsername, password, "权限测试医生", std::nullopt, 2, 1);
  const int disabledUserId = userDao.createUser(
      disabledUsername, password, "禁用测试医生", std::nullopt, 2, 0);
  const int adminUserId = userDao.createUser(
      adminUsername, password, "权限测试管理员", std::nullopt, 1, 1);
  const int doctorRoleCheckUserId = userDao.createUser(
      doctorRoleCheckUsername, password, "角色校验医生", std::nullopt, 2, 1);

  auto activeUser = userDao.findById(activeUserId);
  auto disabledUser = userDao.findById(disabledUserId);
  auto adminUser = userDao.findById(adminUserId);
  auto doctorRoleCheckUser = userDao.findById(doctorRoleCheckUserId);

  EXPECT_TRUE("新建启用用户可查询到", activeUser.has_value());
  EXPECT_TRUE("新建禁用用户可查询到", disabledUser.has_value());
  EXPECT_TRUE("新建管理员用户可查询到", adminUser.has_value());
  EXPECT_TRUE("新建角色校验医生可查询到", doctorRoleCheckUser.has_value());

  {
    AuthResult result = authService.login(activeUsername, password);
    EXPECT_TRUE("启用用户登录成功", result.success);
    EXPECT_TRUE("启用用户登录返回 token", !result.token.empty());
  }

  {
    AuthResult result = authService.login(disabledUsername, password);
    EXPECT_TRUE("禁用用户登录失败", !result.success);
    EXPECT_EQ("禁用用户登录返回 401", result.code, 401);
    EXPECT_EQ("禁用用户登录提示正确", result.msg, std::string("用户已被禁用"));
  }

  if (activeUser.has_value()) {
    const std::string token = AuthSessionManager::issueToken(*activeUser);

    httplib::Request req;
    req.headers.emplace("Authorization", "Bearer " + token);
    httplib::Response res;
    auto verifiedUser = AuthSessionManager::requireUser(req, res);
    EXPECT_TRUE("启用用户 token 可访问受保护接口", verifiedUser.has_value());

    bool updateOk = userDao.updateUser(activeUserId, std::nullopt, std::nullopt,
                                       0, std::nullopt, std::nullopt);
    EXPECT_TRUE("将启用用户改为禁用成功", updateOk);

    httplib::Request disabledReq;
    disabledReq.headers.emplace("Authorization", "Bearer " + token);
    httplib::Response disabledRes;
    auto deniedUser = AuthSessionManager::requireUser(disabledReq, disabledRes);
    EXPECT_TRUE("用户被禁用后旧 token 失效", !deniedUser.has_value());
    EXPECT_EQ("禁用后访问返回 401", disabledRes.status, 401);
    try {
      json body = json::parse(disabledRes.body);
      EXPECT_EQ("禁用后访问提示正确", body["msg"].get<std::string>(),
                std::string("当前用户已被禁用或登录已失效"));
    } catch (...) {
      TestRunner::fail("禁用后访问返回合法 JSON", "响应体不是合法 JSON");
    }
  }

  if (adminUser.has_value()) {
    const std::string adminToken = AuthSessionManager::issueToken(*adminUser);
    httplib::Request req;
    req.headers.emplace("Authorization", "Bearer " + adminToken);
    httplib::Response res;
    auto verifiedAdmin = AuthSessionManager::requireUser(req, res, {1});
    EXPECT_TRUE("管理员 token 通过管理员接口校验", verifiedAdmin.has_value());
  }

  if (disabledUser.has_value()) {
    const std::string disabledToken =
        AuthSessionManager::issueToken(*disabledUser);
    httplib::Request req;
    req.headers.emplace("Authorization", "Bearer " + disabledToken);
    httplib::Response res;
    auto deniedUser = AuthSessionManager::requireUser(req, res);
    EXPECT_TRUE("初始即禁用的用户 token 不可访问接口", !deniedUser.has_value());
    EXPECT_EQ("初始禁用用户访问返回 401", res.status, 401);
  }

  if (doctorRoleCheckUser.has_value()) {
    const std::string doctorToken =
        AuthSessionManager::issueToken(*doctorRoleCheckUser);
    httplib::Request req;
    req.headers.emplace("Authorization", "Bearer " + doctorToken);
    httplib::Response res;
    auto deniedByRole = AuthSessionManager::requireUser(req, res, {1});
    EXPECT_TRUE("普通医生不能通过管理员接口校验", !deniedByRole.has_value());
    EXPECT_EQ("普通医生访问管理员接口返回 403", res.status, 403);
  }

  try {
    DatabaseManager::getInstance().execute(
        "DELETE FROM user_roles WHERE user_id IN (" +
        std::to_string(activeUserId) + ", " + std::to_string(disabledUserId) +
        ", " + std::to_string(adminUserId) + ", " +
        std::to_string(doctorRoleCheckUserId) + ")");
    DatabaseManager::getInstance().execute(
        "DELETE FROM users WHERE id IN (" + std::to_string(activeUserId) +
        ", " + std::to_string(disabledUserId) + ", " +
        std::to_string(adminUserId) + ", " +
        std::to_string(doctorRoleCheckUserId) + ")");
    TestRunner::pass("权限闭环测试产生的临时用户已清理");
  } catch (const std::exception &e) {
    TestRunner::fail("权限闭环测试产生的临时用户已清理", e.what());
  }
}
...
void testEndToEndHealthCheck() {
  ...
  std::thread server_thread([TEST_PORT, &server_ready, &server_stop]() {
    ...
    svr.Get("/api/v1/health-check", [](const httplib::Request &,
                                       httplib::Response &res) {
      ResponseHelper::setCorsHeaders(res);
  ...
  // ── 发送 GET /api/v1/health-check ─────────────────────────
  {
    ...
    if (result) {
      EXPECT_EQ("health-check HTTP 状态码 = 200", result->status, 200);
  ...
  // ── 发送 OPTIONS 预检请求 ─────────────────────────────────
  {
    ...
    if (result) {
      EXPECT_EQ("OPTIONS 预检 HTTP 状态码 = 204", result->status, 204);
    }
  }
...
int main() {
  ...
  // ── 第三部分：认证与权限闭环测试 ───────────────────────────
  std::cout << "\n===== 第三部分：认证与权限闭环测试 =====\n";
  testAuthPermissionClosure();

  // ── 第四部分：模块5/6 Service 参数校验测试 ─────────────────
  std::cout << "\n===== 第四部分：模块5/6 Service 参数校验测试 =====\n";
  testDiseaseAndVisitServiceValidation();

  // ── 第五部分：端到端 HTTP 测试 ─────────────────────────────
  std::cout << "\n===== 第五部分：端到端 HTTP 测试 =====\n";
  testEndToEndHealthCheck();
  testEndToEndModule5And6Routes();
