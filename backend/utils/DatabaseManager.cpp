...
void DatabaseManager::init(const std::string &host, int port,
  ...
  if (!conn_) {
    throw std::runtime_error(
        "MySQL 客户端库初始化失败（mysql_init 返回 NULL）");
  }
  ...
  if (!mysql_real_connect(conn_, host_.c_str(), user_.c_str(),
                          ...
                          static_cast<unsigned int>(port_), nullptr, 0)) {
  ...
  initialized_ = true;
  std::cout << "[DB] 连接成功: " << db_name_ << "@" << host_ << ":" << port_
            << std::endl;
}
...
MYSQL *DatabaseManager::getConnectionUnlocked() {
  if (!initialized_ || !conn_) {
    throw std::runtime_error(
        "数据库未初始化，请先在 main() 中调用 DatabaseManager::init()");
  }
...
MYSQL_RES *DatabaseManager::query(const std::string &sql) {
  std::lock_guard<std::mutex> lock(mutex_);
  MYSQL *conn = getConnectionUnlocked();
