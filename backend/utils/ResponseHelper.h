...
namespace ResponseHelper {
    ...
    /**
     * @brief 构造错误响应体
     * @param code  错误码（400 参数错误 / 401 未认证 / 404 不存在 / 500 服务器错误）
     * @param msg   错误描述信息
     * @return json 对象
     */
     ...
     */
    inline void sendJson(httplib::Response& res,
                         const json& body,
                         int status = 200) {
        res.status = status;
     ...
     * @param msg   提示信息
     */
    inline void ok(httplib::Response& res,
     ...
     * @param msg   错误描述
     */
    inline void fail(httplib::Response& res,
                     int code,
